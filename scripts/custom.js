...
(function(a,b,c){if(c in b&&b[c]){var d,e=a.location,f=/^(a|html)$/i;a.addEventListener("click",function(a){d=a.target;while(!f.test(d.nodeName))d=d.parentNode;"href"in d&&(d.href.indexOf("http")||~d.href.indexOf(e.host))&&(a.preventDefault(),e.href=d.href)},!1)}})(document,window.navigator,"standalone")
	...
	$('#one').click(function(){
		...
		$('#two, #three, #four').removeClass('selected-page-nav');
		$('.two, .three, .four').hide();
		$('.one').fadeIn();
		return false;
	});
	...
	$('#two').click(function(){
		...
		$('#one, #three, #four').removeClass('selected-page-nav');
		$('.one, .three, .four').hide();
		$('.two').fadeIn();
		return false;
	});
	...
	$('#three').click(function(){
		...
		$('#two, #one, #four').removeClass('selected-page-nav');
		$('.two, .one, .four').hide();
		$('.three').fadeIn();
		return false;
	});
    ...
    $('#four').click(function(){
		$('#four').addClass('selected-page-nav');
		$('#two, #one, #three').removeClass('selected-page-nav');
		$('.two, .one, .three').hide();
		$('.four').fadeIn();
		return false;
	});
